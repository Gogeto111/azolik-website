...
function getReply(input: string): string {
  ...
  if (lower.includes('pric') || lower.includes('cost') || lower.includes('plan'))
    return AUTO_REPLIES.pricing;
  if (lower.includes('industr') || lower.includes('sector')) return AUTO_REPLIES.industries;
  if (lower.includes('start') || lower.includes('begin') || lower.includes('fast'))
    return AUTO_REPLIES.started;
  if (lower.includes('work') || lower.includes('how') || lower.includes('do'))
    return AUTO_REPLIES.work;
  ...
  return (
    <>
      ...
      <button
        ...
        style={{
          background: open ? 'rgba(255,255,255,0.08)' : 'linear-gradient(135deg, #7c3aed, #0ea5e9)',
          boxShadow: open ? 'none' : '0 0 40px rgba(124,58,237,0.3), 0 8px 32px rgba(0,0,0,0.4)',
